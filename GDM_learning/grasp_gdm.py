...
import wandb as w # weights and biases
import time
...
class run_gdm(processing):
			...
			if not batch:
				...
				for s in tqdm(scene_folder):
					...
					if check:
						...
						#tmp_dataset = np.array(tmp_dataset, dtype=object) # uncomment if numpy error.
						m_dataset.append(tmp_dataset)
	...
	def run(self, f_data, f_e_label, f_s_label, **kwargs):
		...
		for episode in tqdm(range(episodes)):
			...
			# uncomemt the below lines for episode based training.
			'''if not batch:
				data = f_data
				e_label = f_e_label
				s_label = f_s_label
			...
			if save:
				...
				# save results 
				super().write_file('episodic_error_counter_'+str(episode)+'_{}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+'.npy', episodic_error_counter, path=self.working_dir+'/'+self.save_folder)
				super().write_file('semantic_error_counter_'+str(episode)+'_{}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+'.npy', semantic_error_counter, path=self.working_dir+'/'+self.save_folder)
				super().write_file('episodic_neuron_counter_'+str(episode)+'_{}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+'.npy', episodic_neuron_counter, path=self.working_dir+'/'+self.save_folder)
				super().write_file('semantic_neuron_counter_'+str(episode)+'_{}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+'.npy', semantic_neuron_counter, path=self.working_dir+'/'+self.save_folder)
				super().write_file('episodic_accuracy_counter_'+str(episode)+'_{}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+'.npy', episodic_accuracy_counter, path=self.working_dir+'/'+self.save_folder)
				super().write_file('semantic_accuracy_counter_'+str(episode)+'_{}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+'.npy', semantic_accuracy_counter, path=self.working_dir+'/'+self.save_folder)
				super().write_file('episodic_update_counter_'+str(episode)+'_{}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+'.npy', episodic_update_rate_counter, path=self.working_dir+'/'+self.save_folder)
				super().write_file('semantic_update_counter_'+str(episode)+'_{}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+'.npy', semantic_update_rate_counter, path=self.working_dir+'/'+self.save_folder)
	...
	def test(self,e_net,s_net,test_type='instance', dist_type=None, data_pre_process=False, **kwargs):
		...
		for s in test_scenes:
			...
			global_instance_acc.append(instance_acc) 
			global_category_acc.append(category_acc) 
		...
		super().write_file('global_category_accuracy'+'_{}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+'.npy', global_category_acc, path=self.working_dir+'/'+self.save_folder)
		super().write_file('global_instance_accuracy'+'_{}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+'.npy', global_instance_acc, path=self.working_dir+'/'+self.save_folder)
		super().write_file('global_overall_category_accuracy'+'_{}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+'.npy', global_overall_category_acc, path=self.working_dir+'/'+self.save_folder)
		super().write_file('global_overall_instance_accuracy'+'_{}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))+'.npy', global_overall_instance_acc, path=self.working_dir+'/'+self.save_folder)
