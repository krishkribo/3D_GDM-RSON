...
def run(args, parameters, t):
	...
	if args.mode.lower() == 'both' or args.mode.lower() == 'train':
		...
		if args.continual_learning_mode == 'NI':
			...
			# followed by retraining on 5 objects per bacth
			intial_class_no = 10
			diff = 5
			flag = False
			process.batch = False
			for c in range(3):
				...
				# retrain - 5 objects per batch
				print("Retraining -->")
				print(intial_class_no, intial_class_no+diff)
				#time.sleep(2)
				process.pre_trained = True
				process.g_episodic = e_net
				process.g_semantic = s_net
	...
	"""elif args.mode.lower() == 'both':
		...
		f_data, f_e_label, f_s_label = process.get_data(scene_folder=scene_folder, test_scenes=process.test_scenes,batch=bool(args.mini_batch),batch_size=args.bs_size,train=True,test=False)
		# run training
		e_net, s_net = process.run(f_data, f_e_label, f_s_label)
		...
		# testing 
		#e_net = processing().import_network(e_file, EpisodicGWR)
		#s_net = processing().import_network(s_file, EpisodicGWR)
		if not args.context:
			print(f"Testing without TC ----->")
			e_net.context = False
			s_net.context = False
...
if __name__ == '__main__':
	...
	#if args.preprocess.lower() == 'true': args.preprocess = True
	#elif args.preprocess.lower() == 'false': args.preprocess = False
	parameters = {}
	count = 0
