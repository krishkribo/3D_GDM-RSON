...
class processing(model_preperation):
	...
	def write_file(self,file,value,path):
		...
		#if not path in os.listdir(self.working_dir):
	...
	def get_distance(self, x, y, type='euclidean', **kwargs):
		p_val = kwargs.get('p_value', 2) # for minkowski distance
		x = np.array(x)
		y = np.array(y)
		if type == 'l2norm':
	...
	def model_predict(self, data, **kwargs):
		...
		#print(f"Model to use for feature extraction: {m_name}, model type : {m_type}")
		...
		for s in tqdm(os.listdir(self.dataset_path)):
			...
			for d in tqdm(os.listdir(self.dataset_path+'/'+s)):
				...
				for pcl_file in os.listdir(self.dataset_path+'/'+s+'/'+d):
					...
					depth_image = np.array(depth_image)
					rgb_image = np.array(rgb_image)
					#self.get_entropy_img(depth_image, gray_scale=True, norm=True)
	...
	def plot_network(self, n_net, edges, labels,title,network=None, **kwargs):
		...
		#plt.legend(["lable "+str(i) for i in range(net.num_labels[0])])
		if show_plot:
			plt.show(block=False)
			plt.pause(5)
