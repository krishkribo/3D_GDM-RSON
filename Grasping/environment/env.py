...
class Environment:
	...
	TARGET_ZONE_POS_2 = [-0.7, 0.0, 0.685]
	...
	def __init__(self, camera: Camera, vis=False, debug=False, gripper_type='140', finger_length=0.06) -> None:
		...
		self.tableID = p.loadURDF('Grasping/environment/urdf/objects/table.urdf',
								  [0.0, -0.65, 0.76],
								  p.getQuaternionFromEuler([0, 0, 0]),
								  useFixedBase=True)
		self.target_table_id = p.loadURDF('Grasping/environment/urdf/objects/target_table.urdf',
										  [0.7, 0.0, 0.66],
										  p.getQuaternionFromEuler([0, 0, 0]),
										  useFixedBase=True)
		self.target_id = p.loadURDF('Grasping/environment/urdf/objects/traybox.urdf',
									self.TARGET_ZONE_POS,
									p.getQuaternionFromEuler([0, 0, 0]),
									useFixedBase=True,
									globalScaling=0.7)
		
		self.target_table_id_2 = p.loadURDF('Grasping/environment/urdf/objects/target_table.urdf',
								  [-0.7, 0.0, 0.66],
								  p.getQuaternionFromEuler([0, 0, 1.57]),
								  useFixedBase=True)
		
		self.target_id_2 = p.loadURDF('Grasping/environment/urdf/objects/traybox.urdf',
		...
		self.UR5Stand_id = p.loadURDF('Grasping/environment/urdf/objects/ur5_stand.urdf',
									  [-0.7, -0.36, 0.0],
									  p.getQuaternionFromEuler([0, 0, 0]),
									  useFixedBase=True)
		self.robot_id = p.loadURDF('Grasping/environment/urdf/ur5_robotiq_%s.urdf' % gripper_type,
	...
	def load_isolated_obj(self, path, mod_orn=False, mod_stiffness=False):
		r_x = random.uniform(
			self.obj_init_pos[0] - 0.2, self.obj_init_pos[0] + 0.2)
		r_y = random.uniform(
			self.obj_init_pos[1] - 0.2, self.obj_init_pos[1] + 0.2)
	...
	def create_packed(self, obj_info):
		init_x, init_y, init_z = 0.1, -0.5, self.Z_TABLE_TOP
		yaw = random.uniform(0, np.pi)
