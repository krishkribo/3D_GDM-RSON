...
def plot_network(net, edges, labels,title) -> None:
    """ 2D plot
    """        
    # Plot network
    ...
    print(f"shape of net weights : {np.shape(net.weights)}")    
    print(f"length of net weights : {len(net.weights)}")
    print(f"length of net alabels : {len(net.alabels)}")
    
    ###
    num_connections = 0
    for ni in range(len(net.weights)):
        #print(ni)
        #print(net.alabels[ni])
        """print(np.shape(net.alabels))
        print(ni)
        plindex = np.argmax(net.alabels[ni])
        print(plindex)"""
        
        #print(f"Net alabels :{net.alabels}")
        #print(f"Net alabels[ni] :{net.alabels[ni]}")
        #plindex = np.argmax(net.alabels[ni])
    ...
    plt.title(title)
    plt.xlabel('feature weights 1')
    plt.ylabel('feature weights 2')
    plt.show()
