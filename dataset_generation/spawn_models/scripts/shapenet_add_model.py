'''
Test the 3D model objects using different scale values to fit the object in gazebo world for dataset Generation

change the scale values in the folder structure to fit the object in the Gazebo world
e.g: scale_1, where 1 is the scale size
'''
...
<static>0</static>
    <link name="model_link">
      ...
      <collision name="collision">
        ...
        <geometry>
          <mesh>
            <uri>model://shapenet_models/modelname.dae</uri>
...
def create_model_request(modelname, px, py, pz, rr, rp, ry,scale_size):
    ...
    model = model.replace('scale_size',str(scale_size))
    # Replace modelname
...
if __name__ == '__main__':
    ...
    models = []
    model_dir = os.listdir("/home/krish/project_workspace/workspace/src/spawn_models/models/shapenet_models")
    for m in model_dir:
        if os.path.isfile("/home/krish/project_workspace/workspace/src/spawn_models/models/shapenet_models/"+m):
            models.append(m.split('.')[0])
    #print(models)
    model = models[0]
    print(f"model name : {model}")
    scale_size = float(input("enter scale_size : "))
    # delete_model 
    delete_model_request(model)
    # Spawn object 1
    rospy.loginfo("Spawning model")
    req1 = create_model_request(model,
                              0, 0, 0.003076,  # position
                              0.0, 0.0, 0.0,  # rotation
                              scale_size=scale_size)  # size
    spawn_srv.call(req1)
