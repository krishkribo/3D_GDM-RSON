...
class EpisodicGWR(GammaGWR):
    ...
    def run_egwr(self,epoch,ds_vectors,ds_labels,w,w_logs) -> None:
        ...
        for iteration in tqdm(range(0, samples)):
            ...
            for z in range(1, self.depth):
                self.g_context[z] = (self.beta * self.previous_bmu[z]) + ((1-self.beta) * self.previous_bmu[z-1])
            ...
            if (not self.regulated) or (self.regulated and misclassified):
                ...
                if (a < self.a_threshold
                    ...
                    self.update_temporal(n_index, self.previous_ind, new_node = True)
                    ...
                    w.log({w_logs[1] : b_rate,  w_logs[2] : n_rate})
    ...
    def train_egwr(self, ds_vectors, ds_labels, epochs, a_threshold, beta, 
                   l_rates, context, cpu_count, learning_type, batch, w, w_logs, regulated) -> (list,list,list):
        ...
        if learning_type == 0:
            if batch: self.max_nodes = self.samples
            else: self.max_nodes = 5000
        ...
        config.update({"regulated":regulated})
        ...
        print(f"Number of cpu used for training : {cpu_count}")

        for epoch in tqdm(range(0, self.max_epochs)):
            self.run_egwr(epoch,ds_vectors,ds_labels,w, w_logs[:4])

            # Remove old edges
            super().remove_old_edges()
            
            # Average quantization error (AQE)
            self.error_counter[epoch] /= self.samples
            print ("(Epoch: %s, NN: %s, ATQE: %s)" % (epoch + 1, self.num_nodes, self.error_counter[epoch]))
            
            num_nodes_list.append(self.num_nodes)
            q_error_list.append(self.error_counter[epoch])

            w.log({w_logs[4]:self.error_counter[epoch]})
            w.log({w_logs[5]:self.num_nodes, w_logs[6]:self.ages, w_logs[7]:self.edges})
        ...
        return num_nodes_list, self.num_nodes, q_error_list
